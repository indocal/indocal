...
export interface EmailColumnReportProps {
  report: TableFormFieldColumnReport;
}

export const EmailColumnReport: React.FC<EmailColumnReportProps> = ({
  report,
}) => {
  const content = useMemo(
    () => report.content as EmailFormFieldReport,
    [report]
  );
  ...
  const options: ChartOptions = useMemo(
    () => ({
      chart: {
        id: EmailColumnReport.name + report.column.heading,
        toolbar: { show: true },
      },
      title: {
        style: {
...
export default EmailColumnReport;

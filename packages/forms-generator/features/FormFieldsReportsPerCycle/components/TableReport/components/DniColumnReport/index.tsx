...
export interface DniColumnReportProps {
  report: TableFormFieldColumnReport;
}

export const DniColumnReport: React.FC<DniColumnReportProps> = ({ report }) => {
  const content = useMemo(() => report.content as DniFormFieldReport, [report]);

  const options: ChartOptions = useMemo(
    () => ({
      chart: {
        id: DniColumnReport.name + report.column.heading,
        toolbar: { show: true },
      },
      title: {
        style: {
...
export default DniColumnReport;

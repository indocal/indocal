...
export interface DateColumnReportProps {
  report: TableFormFieldColumnReport;
}

export const DateColumnReport: React.FC<DateColumnReportProps> = ({
  report,
}) => {
  const content = useMemo(
    () => report.content as DateFormFieldReport,
    [report]
  );
  ...
  const options: ChartOptions = useMemo(
    () => ({
      chart: {
        id: DateColumnReport.name + report.column.heading,
        toolbar: { show: true },
      },
      title: {
        style: {
...
export default DateColumnReport;

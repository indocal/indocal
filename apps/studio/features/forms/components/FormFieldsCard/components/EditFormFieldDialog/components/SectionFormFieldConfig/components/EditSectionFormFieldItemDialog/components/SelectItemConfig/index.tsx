...
export interface SelectItemConfigProps {
  item: number;
}

export const SelectItemConfig: React.FC<SelectItemConfigProps> = ({ item }) => {
  const {
  ...
  const {
    ...
    name: `config.items.${item}.config.options`,
  });
          ...
          options.map((option, index) => (
            <ListItem key={option.id} divider>
              ...
              <TextField
                ...
                disabled={isSubmitting}
                inputProps={register(
                  `config.items.${item}.config.options.${index}`
                )}
                error={Boolean(
                  errors.config &&
                    errors.config?.items &&
                    ...
                    errors.config?.items[item]?.config?.options &&
                    ((
                      errors.config?.items[item]?.config?.options as FieldErrors
...
export default SelectItemConfig;

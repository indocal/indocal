  ...
  const onSubmit = useCallback(
    async (formData: FormData) => {
      ...
      if (error) {
        ...
        if (folder.folder) {
          await mutate(`${ApiEndpoints.FOLDERS}/${folder.folder.id}`);
        } else {
          await mutate(`${ApiEndpoints.FOLDERS}/${folder.id}`);
        }
    ...
    [folder.id, folder.folder, mutate, toggleEditFolderDialog, enqueueSnackbar]
  );
